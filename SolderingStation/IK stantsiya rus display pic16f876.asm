; v. 29.11.2016 (добавлена возможность пропуска термопрофиля, пока температура не достигнет заданного значения до которого нагрета плата)
...
#define ON_OFF_ENC      PORTB,RB0     ; назначаем порт для кнопки энкодера
#define RS_LCD          LCD_tmp1,1    ; назначаем вывод команд RS для LCD
#define E_LCD           LCD_tmp1,0    ; назначаем вывод команд E для LCD
#define D7_1        bsf PORTC,RC4     ; назначаем порты для вывода данных на LCD
#define D7_0        bcf PORTC,RC4     ; -//-
#define D6_1        bsf PORTC,RC5     ; -//-
#define D6_0        bcf PORTC,RC5     ; -//-
#define D5_1        bsf PORTC,RC6     ; -//-
#define D5_0        bcf PORTC,RC6     ; -//-
#define D4_1        bsf PORTC,RC7     ; -//-
#define D4_0        bcf PORTC,RC7     ; -//-
#define D3_1        bsf PORTB,RB1     ; -//-
#define D3_0        bcf PORTB,RB1     ; -//-
#define D2_1        bsf PORTB,RB2     ; -//-
#define D2_0        bcf PORTB,RB2     ; -//-
#define D1_1        bsf PORTB,RB3     ; -//-
#define D1_0        bcf PORTB,RB3     ; -//-
#define D0_1        bsf PORTB,RB6     ; -//-
#define D0_0        bcf PORTB,RB6     ; -//-
...
#define PROPUSK         TEMPS,0       ; используем для установок флага необходимости пропуска термопрофиля, пока температура не достигнет заданного значения до которого нагрета плата (1-температура не достигла заданного значения, 0-температура достигла заданного значения)
#define BEEP_ON     bcf PORTA,RA4     ; назначаем вывод для управления зуммером
#define BEEP_OFF    bsf PORTA,RA4     ; назначаем вывод для управления зуммером
...
Go          call        BEEP          ; звуковой сигнал после включения устройства
            ...
            call        T_PROF        ; выводим надпись термопрофиль
            call        OUTPUT_PR     ; выводим на дисплей номер термопрофиля
            call        kursor_OUT    ; выставляем курсор

            call        OFF_PR        ; выводим надпись СТОП напротив ТЕРМОПРОФИЛЬ
...
Goo         bcf         SEC_PAUS      ; опускаем флаг необходимости паузы термопрофиля
            ...
            call        OUT_T_h       ; выводим на дисплей температуру верхней термопары
            call        OUT_T_l       ; выводим на дисплей температуру нижней термопары
...
OUTPUT_Pll  clrf        Reg_tmpH      ; очищаем Reg_tmpH
            ...
            movlw       0x86          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду
...
Phh_UPs     incf        Reg_T_TEMPhh,F ; увеличиваем содержимое Reg_T_TEMPhh
            goto        OUTPUT_Phh    ; выводим на дисплей устанавливаемую мощность для автоматического режима работы верхнего нагревателя(сервисная уставка)
...
OUTPUT_Tl   clrf        Reg_tmpH      ; очищаем регистр Reg_tmpH
            ...
            call        TRANSFORMER_dig ;
            movlw       0xCB          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду
            call        simb_t        ; t
            movlw       0x30          ; подменяем пустым знакоместом , если первый 0 
            movwf       Reg_tmp1      ; -//-
...
OUTPUT_PR   clrf        Reg_tmpH      ; очищаем Reg_tmpH
            ...
            movwf       Reg_tmpL      ; -//-
            call        TRANSFORMER_dig ;
            movlw       0xA1          ; курсор в пределах третьей строки ( 0x94 - начало )
            call        LCD_com       ; загружаем команду
            movlw       0x30          ; подменяем пустым знакоместом , если первый 0 
            movwf       Reg_tmp1      ; -//-
            movf        Tens,W        ; -//-
...
OFF         call        OFFh          ; отключаем верхний нагреватель
            bcf         PROPUSK       ; опускаем флаг необходимости пропуска термопрофиля (0-пропуск не требуется)
...
T_UP_h      bcf         STATUS, Z     ; сбрасываем флаг Z
            bcf         PROPUSK       ; сбрасываем флаг необходимости пропуска термопрофиля (0-пропуск не требуется)
...
Symbol      movlw       0x80          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_up       ; стрелка вверх
            movlw       0x84          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_grad     ; "
            call        simb_C        ; C
            movlw       0xC0          ; курсор в пределах второй строки ( 0xС0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_down     ; стрелка вниз
            movlw       0xC4          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_grad     ; "
            call        simb_C        ; C
            return
...
ABTh        movlw       0x87          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду
            call        simb_A        ; A
            call        simb_B        ; B
            call        simb_T        ; T
            bsf         AV_R_H        ; ставим флаг (перевод в автоматический режим верхнего нагревателя)
            call        READs         ; читаем уставку максимальной мощности для верхнего и нижнего нагревателя для автоматического режима работы
...
RUCHh       movlw       0x87          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду
            call        simb_P        ; P
            call        simb_rus_Y    ; У
            call        simb_rus_CH   ; Ч
...
ABTl        movlw       0xC7          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду
            call        simb_A        ; A
            call        simb_B        ; B
            call        simb_T        ; T
...
RUCHl       movlw       0xC7          ; курсор в пределах первой строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду
            call        simb_P        ; P
            call        simb_rus_Y    ; У
            call        simb_rus_CH   ; Ч
...
ONh         movlw       0x90          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_B        ; B
            call        simb_K        ; К
            call        simb_rus_L    ; Л
...
OFFh        movlw       0x90          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_O        ; О
            call        simb_T        ; T
            call        simb_K        ; К
...
OFFl        movlw       0xD0          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_O        ; О
            call        simb_T        ; T
            call        simb_K        ; К
            ...
            movwf       CCP1CON       ; -//-
            bcf         PORTC,RC2     ; отключаем нижний нагреватель
            bcf         ON_OFF_L      ; сбрасываем флаг включения нижнего нагревателя
            return

; выводим на дисплей слово ТЕРМОПРОФИЛЬ
T_PROF      movlw       0x95          ; курсор в пределах третьей строки ( 0x94 - начало )
            call        LCD_com       ; загружаем команду
            call        simb_T        ; T
            call        simb_E        ; Е
            call        simb_P        ; P
...
ON_PR       movlw       0xA4          ; курсор в пределах третьей строки ( 0x94 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_rus_P    ; П
            call        simb_rus_Y    ; У
            call        simb_C        ; C
...
OFF_PR      movlw       0xA4          ; курсор в пределах третьей строки ( 0x94 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_C        ; C
            call        simb_T        ; T
            call        simb_O        ; О
...
T_PROF_1    call        Pb            ; выводим на дисплей надпись Pb+
            ...
            movlw       0xE0          ; курсор в пределах четвертой строки ( 0xD4 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_1        ; 1
            call        simb_9        ; 9
            call        simb_0        ; 0
...
Pb          movlw       0xD7          ; курсор в пределах четвертой строки ( 0xD4 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_P        ; P
            call        simb_b        ; Ь
            call        simb_plus     ; +
...
; выводим на дисплей слово Pb-
noPb        movlw       0xD7          ; курсор в пределах четвертой строки ( 0xD4 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_P        ; P
            call        simb_b        ; Ь
            call        simb_tire     ; -
...
Tmax        movlw       0xDB          ; курсор в пределах четвертой строки ( 0xD4 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_T        ; T
            call        simb_m        ; m
            call        simb_a        ; a
...
; удаляем все метки
del_metka   movlw       0x86          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_pust     ; пустое знакоместо
            movlw       0x8A          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_pust     ; пустое знакоместо
            movlw       0x8F          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_pust     ; пустое знакоместо
            movlw       0xC6          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_pust     ; пустое знакоместо
            movlw       0xCA          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_pust     ; пустое знакоместо
            movlw       0xCF          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_pust     ; пустое знакоместо
            movlw       0x94          ; курсор в пределах третьей строки ( 0x94 - начало )
            call        LCD_com       ; загружаем команду 
...
; ставим первую метку
metka1      movlw       0x86          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return

; ставим вторую метку
metka2      movlw       0x8A          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return

; ставим третью метку
metka3      movlw       0x8F          ; курсор в пределах первой строки ( 0x80 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return

; ставим четвертую метку
metka4      movlw       0xC6          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return

; ставим пятую метку
metka5      movlw       0xCA          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return

; ставим шестую метку
metka6      movlw       0xCF          ; курсор в пределах второй строки ( 0xC0 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return

; ставим седьмую метку
metka7      movlw       0x94          ; курсор в пределах третьей строки ( 0x94 - начало )
            call        LCD_com       ; загружаем команду 
            call        simb_kurs     ; >
            return
...
BEEEP       movlw       b'00000000'   ; выключаем модуль CCP2
            movwf       CCP2CON       ; -//-
            bcf         PORTC,RC1     ; отключаем верхний нагреватель во время звукового сигнала
            call        BEEP
            call        BEEP
            call        BEEP
            call        BEEP
...
READs       movlw       7             ; считывается содержимое EEPROM-ячейки с адресом 07h
            call        EEP_READ
...
simb_proc   movlw       0x25          ; %
            call        LCD_dig       ; загружаем данные
            return
simb_grad   movlw       0xDF          ; "
            call        LCD_dig       ; загружаем данные
            return
simb_up     movlw       0xD9          ; стрелка вверх
            call        LCD_dig       ; загружаем данные
            return
simb_down   movlw       0xDA          ; стрелка вниз
            call        LCD_dig       ; загружаем данные
            return
simb_A      movlw       0x41          ; A
            call        LCD_dig       ; загружаем данные
...
simb_x      movlw       0x78          ; x
            call        LCD_dig       ; загружаем данные
            return
simb_rus_I  movlw       0xA5          ; И
            call        LCD_dig       ; загружаем данные
            return
simb_rus_L  movlw       0xA7          ; Л
            call        LCD_dig       ; загружаем данные
            return
simb_rus_P  movlw       0xA8          ; П
            call        LCD_dig       ; загружаем данные
            return
simb_rus_Y  movlw       0xA9          ; У
            call        LCD_dig       ; загружаем данные
            return
simb_rus_F  movlw       0xAA          ; Ф
            call        LCD_dig       ; загружаем данные
            return
simb_rus_CH movlw       0xAB          ; Ч
            call        LCD_dig       ; загружаем данные
            return
...
TPR ; работа по термопрофилю
            ...
            bsf         PCLATH, 3     ; -//-
            bsf         PROPUSK       ; поднимаем флаг необходимости пропуска термопрофиля, пока температура не достигнет заданного значения (1-пропуск требуется)
            movf        Reg_tmp_PR,W  ; копируем в W содержимое Reg_tmp_PR
